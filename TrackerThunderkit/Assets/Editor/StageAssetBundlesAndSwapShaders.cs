...
namespace LITEditor
{
    ...
    public class StageAssetBundlesAndSwapShaders : PipelineJob
    {
        ...
        public override Task Execute(Pipeline pipeline)
        {
            ...
            Object[] explicitAssets = assetBundleDefs
        ...
        private static void SwapRealShadersForStubbedShaders(Material[] materialArray)
        {
            ...
            for (int i = 0; i < materialArray.Length; i++)
            {
                ...
                if(current != null)
                {
                    ...
                    {
                        Debug.Log("RealShaderToStubbed key: " + realShader.name + " | out: " + stubbedShaderName);
                        var GUID = AssetDatabase.FindAssets(stubbedShaderName).First();
                        var stubbedShaderPath = AssetDatabase.GUIDToAssetPath(GUID);
                        Debug.Log("Loading stubbed shader at " + stubbedShaderPath);
                        ...
                        {                      
                            ...
                            Debug.Log("Swapped " + current.name + " to stubbed shader - " + current.shader.name);
                        }
                    }
                }
            }
